...
const T4Screen1 = ({ navigation }) => {
  ...
  const [userDataLOCAL_STORAGE, setLocalUserData] = useState(null); // Rename setUserData to setLocalUserData
  const dispatch = useDispatch();
    ...
    const fetchData = async () => {
      try {
        ...
        if (storedData !== null) {
          // Data found, parse it
          const parsedData = JSON.parse(storedData);
          setLocalUserData(parsedData); // Use setLocalUserData
        } else {
          console.log('No data found');
        }
      } catch (error) {
    ...
    <View>
      {userDataLOGIN_CURRENT_REDUX || userDataLOCAL_STORAGE ? (
        ...
        <View style={{ alignItems: 'center' }}>
          <Text style={{ fontSize: 30 }}>User not exists, please Signup or Login......</Text>
          <View style={{ alignItems: 'center', padding: 10 }}>
            <Button title="signup" onPress={() => navigation.navigate('Signup')} />
          </View>
          <View style={{ alignItems: 'center', padding: 10 }}>
